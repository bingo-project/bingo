...
```mermaid
graph TD
    User[用户] -->|HTTP/WebSocket| Handler
    Handler -->|Request| Biz[AI Biz Layer]
    
    subgraph "Context Management"
        Biz -->|Load History| DB[(MySQL)]
        Biz -->|Calc Token| Utils[Token Counter]
        Biz -->|Sliding Window| Biz
    end
    
    subgraph "Quota System"
        Biz -->|Reserve TPD| Redis[(Redis)]
        Redis -->|Grant/Deny| Biz
    end
    
    subgraph "Provider Layer"
        Biz -->|Get Provider| Registry[AI Registry]
