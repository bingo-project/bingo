...
// If logger is nil, gorm's default logger will be used.
func NewMySQL(opts *MySQLOptions, logger gormlogger.Interface) (*gorm.DB, error) {
	cfg := &gorm.Config{
	...
	db, err := gorm.Open(mysql.Open(opts.DSN()), cfg)
	if err != nil {
		return nil, err
	}
